...
public class Hand {
	...
	public Hand EvaluateHand() {
		...
		if(ExplodedHands.get(0).getCardsInHand().size() != 5){
			try {
				throw new HandException("you try to evaluate a hand with != 5 cards");
	...
	public static ArrayList<Hand> ExplodeHands(Hand h) {
		...
		ReturnHands.add(h);
		ArrayList<Card> cards = h.getCardsInHand();
		//loop each card , in order to judge if there is one joker / Wild
		for(Card card : cards){
			if(card.isWild() || card.geteRank() == eRank.JOKER || card.geteSuit() == eSuit.JOKER ){
